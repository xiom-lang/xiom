// XIOM — Self-Hosted Compiler v0.11.0
      ...
      if c2 == 32 || c2 == 9 {
        ...
        while !(pdone) {
          if (pos >= len) { pdone = 1 == 1; }
            ...
            var pc_char = axiom_char_at(&src, &pos);
            if pc_char == 41 { pdone = 1 == 1; }
            elif pc_char == 44 { comma_count = comma_count + 1; pfound = 1 == 1; }
            elif pc_char >= 65 && pc_char <= 90 { pfound = 1 == 1; }
            elif pc_char >= 97 && pc_char <= 122 { pfound = 1 == 1; }
          }
          if !(pdone) { pos = pos + 1; }
        }
        var param_count = 0;
        ...
        while !(bws_done) {
          if (pos >= len) { bws_done = 1 == 1; }
            ...
            var bws_c = axiom_char_at(&src, &pos);
        ...
        axiom_fn_table_add(&name_id, &ret_type_id, &param_count, &body_start, &body_end);
        fn_count = fn_count + 1;
      } else { pos = pos + 1; }
    }
    elif c0 == 114 && c1 == 101 {
...
fn main() -> Int {
  ...
  axiom_set_source(src + 0);

  // Emit IR for all functions (needs source for body parsing)
  emit_program();

  // Free source AFTER emission
  axiom_free(&src);
  0;
  return 0;
}
