...
mod tests {
    ...
    #[test] fn debug_parse_ensure_expr_body() { let prog = parse("fn identity_non(x: Int) -> Int\n  ensures: result == x\n{\n  x\n}\n").unwrap(); match &prog.items[0] { TopDecl::Fn(f) => { eprintln!("contracts={} body={:?}", f.contracts.len(), f.contracts.iter().map(|c| format!("{:?}", c)).collect::<Vec<_>>()); assert!(f.body.is_some(), "expr body must be Some; contracts={}", f.contracts.len()); } _ => panic!("expected function"), } }
