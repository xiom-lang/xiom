...
/// Run Rust axiomc on a source file with --emit-ir, return IR output lines
fn rust_ir(source: &str) -> Vec<String> {
    let output = Command::new(axiomc_path())
        .args(["--emit-ir", source])
        .current_dir(project_root())
        .output()
        .expect("rust axiomc failed");
...
///
/// Creates a temp copy of selfhost/axiomc_v10.ax with the source path
/// replaced to point at the desired example, compiles it with axiomc,
...
// (using axiomc_v10.ax + C runtime emit_body_ir) differs from the full
// Rust compiler:
//
...
diff_test!(diff_demo_float, "demo_float.ax", 3);
diff_test!(diff_diff_test, "diff_test.ax", 1);
diff_test!(diff_ownership, "phase1_ownership.ax", 3);
diff_test!(diff_async, "phase1_async.ax", 2);
diff_test!(diff_async_spawn, "phase1_async_spawn.ax", 2);
diff_test!(diff_modules, "phase1_modules.ax", 1);
diff_test!(diff_interface, "phase1_interface.ax", 1);
diff_test!(diff_selfhost, "phase1_selfhost.ax", 1);
diff_test!(diff_stress_borrow, "stress_borrow_10level.ax", 10);
diff_test!(diff_stress_float, "stress_float_matrix.ax", 1);

// Derive/enum — Rust generates derived fns; selfhost also generates some via C runtime
diff_test!(diff_derive, "phase1_derive.ax", 3, 0.5, 0.4);
diff_test!(diff_generics, "phase1_generics.ax", 1, 0.5, 0.4);
diff_test!(diff_enum, "phase1_enum.ax", 2, 0.5, 0.4);
diff_test!(diff_derive_enum, "phase1_derive_enum.ax", 1, 0.5, 0.4);
diff_test!(diff_full, "phase1_full.ax", 3, 0.5, 0.4);

// Contracts — Rust handles contract codegen; selfhost also emits invariant_check etc.
diff_test!(diff_contracts, "phase1_contracts.ax", 2, 0.5, 0.4);
...
diff_test!(diff_error, "phase1_error.ax", 0, 0.0, 0.0, 3.0, 3.0, 0.0);
...
diff_test!(diff_hardening, "phase1_hardening.ax", 5, 0.3, 0.2, 3.0, 3.0, 0.3);
...
diff_test!(diff_stress, "phase1_stress.ax", 4, 0.4, 0.4);
...
diff_test!(diff_stress_derive, "stress_derive_50field.ax", 1, 0.5, 0.4);
