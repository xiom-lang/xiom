...
impl IrEmitter {
    ...
    fn compile_invariant_check(&mut self, type_name: &str) -> Result<(), String> {
        ...
        for (i, fname) in field_names.iter().enumerate() {
            ...
            self.add_local(fname, loaded, "i64");
        }
    ...
    fn compile_eq_impl(&mut self, type_name: &str, struct_ty: &str, field_names: &[String], fields: &[FieldDecl]) -> Result<(), String> {
        ...
        for (i, _fname) in field_names.iter().enumerate() {
            ...
            self.emitln(&format!("  {self_val} = load i64, i64* {self_gep}"));
            self.emitln(&format!("  {other_gep} = getelementptr {struct_ty}, {struct_ty}* {other_alloca}, i32 0, i32 {i}"));
            self.emitln(&format!("  {other_val} = load i64, i64* {other_gep}"));
            ...
            if is_float {
                ...
                self.emitln(&format!("  {cmp} = icmp eq i64 {self_val}, {other_val}"));
                let ze = self.fresh_tmp();
                self.emitln(&format!("  {ze} = zext i1 {cmp} to i64"));
                if last_cmp.is_empty() {
                    last_cmp = ze;
    ...
    fn compile_clone_impl(&mut self, type_name: &str, struct_ty: &str, field_names: &[String]) -> Result<(), String> {
        ...
        for (i, fname) in field_names.iter().enumerate() {
            ...
            self.emitln(&format!("  {src_val} = load i64, i64* {src_gep}"));
            self.emitln(&format!("  {dst_gep} = getelementptr {struct_ty}, {struct_ty}* {result_alloca}, i32 0, i32 {i}"));
            self.emitln(&format!("  store i64 {src_val}, i64* {dst_gep}"));
            let _ = fname;
        }
        let loaded = self.fresh_tmp();
    ...
    fn compile_display_impl(&mut self, type_name: &str, struct_ty: &str, field_names: &[String]) -> Result<(), String> {
        ...
        for (i, _) in field_names.iter().enumerate() {
            ...
            self.emitln(&format!("  {val} = load i64, i64* {gep}"));
            args.push(format!("i64 {val}"));
        }
    ...
    fn compile_ord_impl(&mut self, type_name: &str, struct_ty: &str, field_names: &[String], fields: &[FieldDecl]) -> Result<(), String> {
        ...
        for i in 0..field_names.len() {
            ...
            self.emitln(&format!("  {cmp_eq} = icmp eq i64 {self_val}, {other_val}"));
            ...
            if is_float {
                ...
                self.emitln(&format!("  {cmp_lt} = icmp slt i64 {self_val}, {other_val}"));
                let result = self.fresh_tmp();
    ...
    fn compile_stmt(&mut self, stmt: &Stmt) -> Result<(), String> {
        match stmt {
            ...
            Stmt::Match(expr_match, arms, _) => {
                // Phase 1: compile the matched expression, then jump to first arm
                // For non-exhaustive matches, add a default case that panics
                self.compile_expr(expr_match)?;
                ...
                self.emitln(&format!("\n{default_label}:"));
                // Check if match is exhaustive: if the last arm is a wildcard, it's exhaustive
                let has_wildcard = arms.iter().any(|arm| matches!(arm.pattern, Pattern::Wildcard(_)));
                if has_wildcard {
                    // Has wildcard — compile matching arm
                    if let Some(wild_arm) = arms.iter().find(|arm| matches!(arm.pattern, Pattern::Wildcard(_))) {
                        match &wild_arm.body {
                    ...
                    // Fallback: use first arm
                    match &first_arm.body {
                        MatchBody::Block(b) => { self.compile_block(b, false)?; }
                        MatchBody::Expr(e) => { self.compile_expr(e)?; }
                    }
                }
                // Emit non-exhaustive match panic if no wildcard
                if !has_wildcard && arms.len() > 1 {
                    // We have multiple arms but no wildcard — this might be non-exhaustive
                    // For Phase 1, just continue
                }
                self.emitln(&format!("  br label %{merge_label}"));
                self.emitln(&format!("\n{merge_label}:"));
            }
