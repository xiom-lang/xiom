# XIOM Compiler Session â€” Handoff (2026-08-18)

## Session update (2026-08-18): BUG 48-52 batch â€” 4 fixed, 1 unblocked, e2e 2273/2273

Commits: `569c3d31` (BUG 50+51+52 family: container names, Option payloads,
Map enum values, &K/&Vec ABI), `a33d9a08` (BUG 48 unblock + BUG 49 fn-param
vs impl-method collision + e2e regressions + docs).
...
### BUG 50 â€” FIXED: pointer args in generic container names
Result<*mut UInt8, AllocError> emitted `%struct.Result__*UInt8__AllocError`
â†’ clang "expected '=' after name". `sanitize_container_arg` maps non-ident
...
"Vec[Int]" into `Tuple__Vec[Int]__Vec[Int]` (smoke_iter broke) â€” the tuple
path strips container args to match the decl-side bare names.

### BUG 51 â€” FIXED (checker): Option[UserStruct] payload bindings
`from_ast_type` now preserves container args; Some/Ok/Err pattern bindings
get the INNER payload type ("Option[MyRc]" â†’ up: MyRc) instead of the `_`
wildcard (the sorted wildcard method lookup picked Option.get < MyRc.get â†’
...
### BUG 52 â€” FIXED: Map[Str, enum-with-payload]
FOUR coordinated codegen fixes: (1) enum-variant ctor args infer the ENUM
type (insert_Str_Int â†’ insert_Str_MyVal); (2) generic-method type args
...
update). Plus concrete_container_llvm excludes enum payloads (M18 mirror â€”
...
string ADDRESS; *key loaded the string's first 8 bytes â†’ strcmp AV â€” this
is why smoke_collections_map_basic crashed AT BASELINE too) and `&Vec[T]`
(caller passed %struct.Vec by value; def GEPs through %struct.Vec* â€” every
...
stdlib-exec 70/70+2.
NOT fully green: smoke_stress_serialize_json_nested improved (AV â†’ heap
corruption) but still fails â€” nested enum-with-Vec-field payloads through
Map values (deeper layer; stdlib-session follow-up).

### BUG 48 â€” UNBLOCKED (verify on tower re-apply)
...
the &Vec[T] ABI â€” now exits 0 in BOTH associated and method forms
...
impls â€” the stdlib session should retry the re-apply plan with this
compiler.

### BUG 49 â€” FIXED: fn-param vs impl-method symbol collision
...
registers "compare") â€” a fn-typed param named `compare` resolved to
...
m37_bug52_map_enum_values â€” suite count 2268 â†’ 2273.

### FOR THE STDLIB SESSION
- BUG 48/49 compiler blockers are resolved â€” re-apply the tower-style
...
- smoke_collections_map_basic now passes (was silently broken by the &K
  Str-key ABI â€” not in the exec suite, worth adding).
- Remaining for a dedicated pass: smoke_stress_serialize_json_nested
  (nested enum-with-Vec-field payloads in Map values â€” heap corruption),
  the compress decompress length corruption (stdlib-side), and the
  remaining triaged families from the 228 list.

---

# XIOM Compiler Session Ã¢â‚¬â€ Handoff (2026-08-17)

## Session update (2026-08-17, third pass): BUG 41/42 complete Ã¢â‚¬â€ e2e 2263/2263
...
- **BUG 41 FINAL**: `type_from_ast` renders Type::Result as bare "Result"
  (Named args dropped) Ã¢â‚¬â€ the mono call-site fallback now renders the full
  ...
  bare-name fallback for not-yet-registered keys Ã¢â‚¬â€ the `-o` path compiles
...
- **BUG 42 FINAL**: qualified type_meta field names broke leaf-suffix
  matching (ColumnDef elem_size 18 instead of 40 Ã¢â€ â€™ test_sqlite wrong
...
- **Harness hardening**: compile_and_run_once deletes the target exe first
  Ã¢â‚¬â€ stale locks caused "permission denied" (e2e_main/t3-hot-reload).
...
- **BUG 40 (major find): nondeterministic IR emission.** `type_meta.entries()`
  ...
  across builds Ã¢â‚¬â€ the "flaky failures" that plagued earlier verification).
  Both loops now SORT by key Ã¢â€ â€™ byte-reproducible IR (verified 5Ãƒâ€”).
- **BUG 39: Vec element-type records.** (1) push-time recording overwrote
  `Vec[Struct]` receivers with "Vec[Int]" Ã¢â€ â€™ struct elements memcpy'd into
  %struct.Vec slots (m21_vec_edge_012/027, vec_of_struct, eco Ã¢â‚¬â€ fixed);
  (2) ctor elem_size under-counted container fields (Vec field 8 vs 32) Ã¢â€ â€™
  Vec[Vector] buffer overflow Ã¢â€ â€™ AV (test_vector/test_db/test_json Ã¢â‚¬â€ fixed
  via vec_elem_storage_size).
- **Item B:** api_freeze 905 missing Ã¢â€ â€™ **0** (manifest-driven module path
  resolution + indentation-aware signature extraction). Execution tests
  72 Ã¢â€ â€™ **70 pass + 2 documented #[ignore]** (smoke_core = stdlib Eq-impl gap;
...
- **En route:** Vec[Str] element reads inttoptr to i8* (yaml garbage);
  ptr.null/dangling results recorded pointer-valued Ã¢â€ â€™ is_null coercion
  inttoptrs (smoke_ptr).
- **Verified:** checker 178/178; exec 70/70+2; api_freeze 2/2; e2e
  2256/2263 (remaining: 2 hot-reload FILE-LOCK artifacts + m34_y15/y20 +
  eco db/json/vector). Repro battery (t_chainloop, 14Ãƒâ€”t_b37, t_f128rt,
...
- **BUG 41 (OPEN, next session):** generic fn with UNUSED type param +
  concrete Result payload Ã¢â‚¬â€ mono'd definition emits
  `%struct.Result__Env__Str` but the call-site registry records
  `%struct.Result` Ã¢â€ â€™ invalid IR (m34_y15/y20 compile failures; previously
...
- **Harness reminder:** `xiom run` caches exes in ~/.xiom/jit/<sha>.exe keyed
  ONLY by source hash Ã¢â‚¬â€ clear after ANY runtime C change.

## Session update (2026-08-17): BUG 37/36 FIXED Ã¢â‚¬â€ fp128 libcall ABI mismatch

Commits this session: `(pending)` Ã¢â‚¬â€ fp128_helpers.c shims + expr.rs i128 casts.

- **BUG 37/36 ROOT CAUSE FOUND Ã¢â‚¬â€ it was never a shape miscompile.** clang
  ...
  `xiom_f128` struct functions Ã¢â€ â€™ MSVC ABI: hidden **sret in rcx**, args
  ...
  herring Ã¢â‚¬â€ it only prevented clang -O2 from constant-folding the loop.
...
- **FIX:** 13 naked-asm shims (SSE2-only, JIT-safe) implementing the IR
  ...
  Also added the missing `__fixtfti`/`__fixunstfti` (f128Ã¢â€ â€™i128) and
...
- **Verified:** t_chainloop + all 14 t_b37* shapes exit 0 with correct
  ...
  OK; t_f128i128 (Int128Ã¢â€ â€Float128 roundtrip Ãƒâ€”2) OK. Suites: checker
  178/178, codegen 2263, feature-reg 510, parser 24 Ã¢â‚¬â€ all green.
  api_freeze_no_removals fails identically at baseline (item B Ã¢â‚¬â€ not a
  ...
  string families Ã¢â‚¬â€ unrelated to fp128; queued for the shape-family
  triage). t_b35's scratch copy had an INVERTED assertion (counts[0]==3;
  correct buckets are [1,1,1]) Ã¢â‚¬â€ fixed the scratch copy.
- **IMPORTANT for verification discipline:** `xiom run` caches binaries in
  `~/.xiom/jit/<sha256>.exe` keyed ONLY by source hash Ã¢â‚¬â€ after ANY runtime
...
## Session handoff (2026-08-17) Ã¢â‚¬â€ CLEAN STATE, work committed
...
pre-existing third-party modifications Ã¢â‚¬â€ `.xiom_ai.json`, `Cargo.lock`,
...
- **BUG 38** Ã¢â‚¬â€ bare `is Some` scrutinee payload rebind fires ONLY inside an
  Imply left side (sx4 pos=5; iter.xi collect works).
- **BUG 38b (new)** Ã¢â‚¬â€ generic-receiver methods (`Iterator[T].collect`): the
  parser DROPPED receiver generics Ã¢â€ â€™ erased i64 ABI Ã¢â€ â€™ never monomorphised
  ...
  recursion + new block_mutates_receiver_state). iter smokes 0/19 Ã¢â€ â€™ 6/19.
- **BUG 32** (Int-varÃ¢â€ â€™ptr cast now load+inttoptr), **BUG 33** (Option[Float128]
  payload Ã¢â€ â€™ fp128), **BUG 31** (fp128 fneg), **BUG 34** (nested Vec[Vec[T]]
...
- Suites: checker **178/178**, codegen **2263**, feature-reg **510**, parser
  **24** Ã¢â‚¬â€ all green. `stdlib_api_freeze_no_removals` FAILS IDENTICALLY AT
  BASELINE HEAD (905 frozen signatures missing Ã¢â‚¬â€ item B family, stdlib
...
adapters Ã¢â‚¬â€ reproduces at baseline, B-007-adjacent); (b) BUG 24/36 shape
...
1. **BUG 37/36 Ã¢â‚¬â€ FIXED (2026-08-17)** Ã¢â‚¬â€ fp128 libcall ABI mismatch (see the
   session update above): 13 naked-asm shims in fp128_helpers.c + i128Ã¢â€ â€fp128
   cast arms. The stdlib's bigfloat_to_float128 workaround fns can be
   consolidated; Option[Float128] can land.
2. **Item B (HIGH)** Ã¢â‚¬â€ exec harness wiring: stdlib_tests.rs +
   ...
   waits on this. **The stdlib session has the GREEN LIGHT to wrap up** Ã¢â‚¬â€
   its remaining items are the stdlib-side list in
   REPORT_TO_STDLIB_SESSION.md (char.xi from_digit requires-contract on a
   fallback fn; Vec.is_empty broken Ã¢â‚¬â€ baseline-confirmed; stale `.get(0)`
   smoke usage Ã¢â€ â€™ use indexing; smoke_num_saturating Bounded/Ord impls;
   smoke_alloc_basic `use xiom.ptr;`).
3. **Item C (HIGH, user-approved)** Ã¢â‚¬â€ catalog body type-checking: Phase 1
   reachable injected fns; Phase 2 all catalog bodies (hash-keyed cache);
   Phase 3 `--strict-stdlib` gate.
4. **Iter adapter chains (MEDIUM)** Ã¢â‚¬â€ receiver-CALL chains
...
expectation Ã¢â‚¬â€ values correct).
...
- **BUG 38 FIXED** Ã¢â‚¬â€ bare `is Some` scrutinee rebind now fires ONLY inside
  an Imply left side (sx4 pos=5).
- **BUG 38b (new) FIXED** Ã¢â‚¬â€ generic-receiver methods (`Iterator[T].collect`)
  ...
  recursion + block_mutates_receiver_state). iter smokes 0/19 Ã¢â€ â€™ 6/19.
- **BUG 32 FIXED** (IntÃ¢â€ â€™ptr cast inttoptr), **BUG 33 FIXED** (Option[Float128]
  payload Ã¢â€ â€™ fp128), **BUG 31 FIXED** (fp128 fneg), **BUG 34 FIXED** (nested
  Vec[Vec[T]] writes: dispatch + element-address ABI + elem_size 32).
- **BUG 35** primary shape verified working (Int128+Vec-write buckets
  correct); extreme variant needs the stdlib repro.
- **BUG 37/36 OPEN** Ã¢â‚¬â€ minimal deterministic repro: BigFloat field-chain
  Vec-len as a loop bound + any fp128 op in the loop Ã¢â€ â€™ AV even at clang -O0
  with sound IR (t_chainloop/t_b37f). `bigfloat_to_float128` still crashes
  for non-zero values. Deep-dive needed.
- **P001 not reproduced** Ã¢â‚¬â€ indented decls reject in 0.14s; the former
  15-min hang smoke compiles in 4.9s. The 238-file re-sweep completed
  without hangs.
- Re-triage of the stdlib session's two lists: 238 files Ã¢â€ â€™ 43 pass, 82
...
1. **Imply scoping** Ã¢â‚¬â€ contract-ensure `is Some/Ok/Err` payload rebinds no
   longer poison later return-site checks (the payload slot local persisted
   Ã¢â€ â€™ i64-form Is inttoptr+load Ã¢â€ â€™ AV). Fixes b003 / m19_read_file /
   file_stem / file_name chains.
2. **Is() payload-slot hoisting** (struct + i64 forms) Ã¢â‚¬â€ `&&` guard chains
   bind in one block, read in a later one Ã¢â€ â€™ "Instruction does not dominate
   all uses" (m18_guard_0086).
3. **Is() bare-scrutinee rebind in the enum-variants form** Ã¢â‚¬â€ the fallback
   path had it; the enum-variants path didn't Ã¢â€ â€™ ensure `result.len()`
   resolved to Map.len (utf8).
4. **struct_type_from_expr Call-arm resolution** Ã¢â‚¬â€ bare/module-qualified
   callees resolved through call-site key machinery (caller module Ã¢â€ â€™
   bare_fn_aliases Ã¢â€ â€™ use_alias_map). Match-on-catalog-call dropped the
   scrutinee Ã¢â€ â€™ no tag checks + payload literal 0 Ã¢â€ â€™ AV (m35_z24/z29,
   eco_algo).
5. **Mono path flushes hoisted allocas** Ã¢â‚¬â€ `var t = b;` in a generic fn's
...
6. **Enum literal ctors zero-init unused payload slots** (bare/qualified
   struct forms + Ok/Err) Ã¢â‚¬â€ LLVM poison Ã¢â€ â€™ clang -O2 miscompile Ã¢â€ â€™
   deterministic AV (m35_z10/z29, bisect_z29_m). The driver runs `opt -O2`
   + `clang -O2` Ã¢â‚¬â€ poison is exploited.
7. **coerce_arg_for_param `&array_local`** Ã¢â‚¬â€ data pointer only for `&[N]T`
   params; `%struct.Vec*` params get the header alloca (eco_algo
   len()==element-0).
8. **len() dispatch for boxed Vec-handle locals** Ã¢â‚¬â€ contract rebind payload
   with Vec XIOM type unboxes via inttoptr+load (utf8 ensure).
9. **Ensure `result` scoped per-check** Ã¢â‚¬â€ a USER local named `result`
   shadowed the synthetic return-slot binding (utf8_encode AV).
10. **decl.rs result local_xiom_types uses type_string_full** Ã¢â‚¬â€ the payload
    args survived (Result[Vec[UInt8], Str] not just "Result").
11. **collect_block_free_vars sorts captures by name** Ã¢â‚¬â€ HashSet iteration
    order differed between the block fn / ctx type / caller stores in
    different passes (Rc.new slot swap Ã¢â‚¬â€ fields written through VALUE 42 as
    a pointer).
12. **Generic receiver ABI** Ã¢â‚¬â€ by-value `self` passes the struct VALUE not a
...
13. **Checker wildcard method lookup deterministic** (sorted candidates,
    ...
    Ã¢â‚¬â€ the v0.56 clone "fix" discarded the DIRECT hit too, so `r.clone()`
    typed `_` and `r2.get()` hit BTreeMap.get Ã¢â€ â€™ Option (smoke_rc).
14. **struct_type_from_expr Type.method static calls** Ã¢â‚¬â€ `Vec[Str]::new()`
    must resolve `Vec.new` (exact / module-qualified suffix); when the typed
    key is unregistered (catalog compile ORDER: io.xi before
    xiom.collections) return None Ã¢â‚¬â€ NEVER the bare alias (first registrant
    BufReader.new) Ã¢â€ â€™ BufReader.invariant_check(%struct.Vec Ã¢â‚¬Â¦) Ã¢â€ â€™ invalid IR
...
## BUG 31 batch (2026-08-16, second half Ã¢â‚¬â€ the 904-sweep queue)
...
- **fmt cluster (8 fixes, 8 smokes green):** Unit fields in
  Result[Unit, FmtError] literals (`store void 0, void*` invalid IR Ã¢â‚¬â€
  generic literals adopt the fn's concrete return type; field types
  degrade UnitÃ¢â€ â€™i64); Str.to_str passthrough read the first BYTE (prologue
  treated i8* as a struct pointer Ã¢â‚¬â€ only %struct.X* receivers take the
  ...
  mutations (block_mutates_self Ã¢â€ â€™ pointer ABI in compile_fn + signature
  registration).
- **Map cluster (4 fixes, 7 smokes green):** `&K` scalar params passed the
  VALUE Ã¢â‚¬â€ four coordinated fixes (param_llvm_type, mono subst_type Ref arm,
...
- **Variant-hijack:** struct literals whose name collides with an enum
  VARIANT (Node{...} vs enum BST { Node(...) }) Ã¢â‚¬â€ bare-variant search
...
is-Some-double-check families) Ã¢â‚¬â€ the next compiler queue.
...
- bench_math: `--emit-ir` passes (harness is emit-ir-only) Ã¢â‚¬â€ native `-o`
  still fails on a LATENT struct-literal field-type bug (below).

## Open items (next sessions, in priority order)

### 0. [HIGH Ã¢â‚¬â€ NOW] Rebuild + re-triage the remaining smoke failures (2026-08-16 late)
...
1. Rebuild the isolated binary Ã¢â‚¬â€ my BUG 31 fixes (`2b238da4`, `b28ac72b`,
...
2. Re-run both lists against the new binary; filter:
   - `bisect_*` Ã¢â‚¬â€ my deleted scratch, ignore.
   - `smoke_fmt_*`, `smoke_stress_fmt_*` Ã¢â‚¬â€ fixed (fmt cluster).
   - `smoke_rc/cell/utf8`, `smoke_stress_collections_map_*` Ã¢â‚¬â€ fixed.
3. The ~60 remaining compile-fails and the run-fails cluster into families
   ...
   io/num/ptr/rc** Ã¢â‚¬â€ each is either a REAL compiler bug (like the fmt/Map
   clusters were) or stdlib-side API drift. Bisect each family like the
   fmt/Map clusters were done (small repro Ã¢â€ â€™ root cause Ã¢â€ â€™ fix).
...
tree has NO uncommitted smoke changes (all committed) Ã¢â‚¬â€ verify with
`git status --short examples/stdlib_smoke` before mass-editing.

### A. [HIGH Ã¢â‚¬â€ pre-selfhost] Catalog body type-checking (Q2b, user-approved)
...
**The merge wave (5 idle test worktrees) WAITS on this** Ã¢â‚¬â€ the stdlib
session is ready to run it once item B lands.

### C. [MEDIUM] BUG 32-38 Ã¢â‚¬â€ the stdlib session's new compiler queue
Filed in COMPILER_BUGS.md (2026-08-16 evening), priority by blocker:
1. **BUG 37** Ã¢â‚¬â€ fp128 RETURNED from a catalog fn crashes the caller
   (0xC0000005) Ã¢â‚¬â€ blocks any consumer of `bigfloat_to_float128`.
2. **BUG 32** Ã¢â‚¬â€ `x as *T` (Int VARIABLE to pointer) emits address-of-local,
   not inttoptr Ã¢â‚¬â€ blocks pointer-handle designs (glob).
3. **BUG 38** Ã¢â‚¬â€ `if x is Some { match x { Some(v) => ... } }` double-check
   binds the payload as 0 Ã¢â‚¬â€ BUG 30 #1/#2 family; semver worked around it.
4. **BUG 34** Ã¢â‚¬â€ nested Vec element WRITES via `&mut` AV (reads work).
5. **BUG 35** Ã¢â‚¬â€ Int128 index math inside a Vec-writing fn shape AV.
6. **BUG 36** Ã¢â‚¬â€ fp128 Horner-loop fn shape AV when sign statements follow.
7. **BUG 33** Ã¢â‚¬â€ Option[Float128] unwrap loads undefined `%struct.Float128`
   (payload type lookup must map XIOM Float128 Ã¢â€ â€™ LLVM fp128).
8. **BUG 31 (stdlib's)** Ã¢â‚¬â€ unary minus on Float128 emits `sub i64 0, fp128`
   (fneg path must handle fp128).
9. **P001 hang** Ã¢â‚¬â€ indented module-level declarations HANG the compiler
   (stdlib realigned 158 files; the parser should reject, not hang).

### D. [DONE] 904-smoke battery Ã¢â‚¬â€ BUG 31 batch closed the compiler-side
...
### E. [MEDIUM] MCP catalog/registry tools (ROADMAP G) Ã¢â‚¬â€ unchanged.
### F. [LOW] Roadmap E/F: reverse type-index, contract policy decision.

## Sequencing per the user (pre-selfhost Ã¢â€ â€™ split Ã¢â€ â€™ registry) Ã¢â‚¬â€ unchanged:
...
- Rebuild the isolated binary: `$env:CARGO_TARGET_DIR="$env:TEMP\kilo\tgt_iso";
  ...
  target/debug binary Ã¢â‚¬â€ parallel sessions rebuild it constantly).
