...
## 0. SAFETY ARCHITECTURE -- "FEARLESS CONCURRENCY" [OK]

### 0.1 What's Implemented (Domain B -- Language Threading)

| Feature | Status | Implementation |
|---------|--------|---------------|
| **spawn codegen** | [OK] DONE | Body compiled as separate LLVM function `@_xiom_spawn_N`. Called via `@xiom_thread_spawn(ptr @fn, ptr null)`. Module-level declare. |
| **Send/Sync markers** | [OK] DONE | `interface Send { }` / `interface Sync { }` registered in checker. Auto-derived by compiler from field composition. All primitives are Send+Sync. |
| **Channel[T]** | [OK] DONE | Bounded MPSC ring buffer (64 slots) in C runtime. Mutex + condition variable. `create/send/recv/try_recv/close`. |
| **Thread pool** | [OK] DONE | Work-stealing worker threads in C runtime. `xiom_threadpool_init(N)`, `xiom_threadpool_spawn(fn, arg)`. Auto-scales to CPU count. |
| **C runtime threading** | [OK] DONE | `xiom_thread_create/join/detach`, `xiom_mutex_*`, `xiom_cond_*` on Windows + Linux. 256-slot spawn tracking table. |
| **Spawn wrapper functions** | [OK] DONE | Body emitted as separate function via save/restore of `self.output`. Flushed via `deferred_closure_defs`. |
...
|-----|--------|---------------|
| **Move semantics for spawn** | [OK] DONE | `spawn move { ... }` -- capture analysis via `collect_expr_references_block()`, heap env struct forwarding (flat i64 array at offset*8), move-after-spawn prevention (scope removal). |
| **Thread-local recursion counter** | [OK] DONE | `@xiom_recursion_counter = internal thread_local global i64 0` since emitter.rs inception. |
| **Spawn capture layout** | [OK] DONE | Captured variables packed as i64 sequence in malloc'd env buffer. Spawn function unpacks via getelementptr+load. |
| **Parallel codegen** | [OK] DONE | `--parallel-codegen` flag. Rayon-based per-function IR emission with output merging in declaration order. |
...
| Phase | What | Status |
|-------|------|--------|
| **A1: Parallel Parse** | Parse files concurrently | [OK] DONE -- `--parallel` flag, `rayon::par_iter()` |
| **A2: Dependency Graph** | Build import graph, topological sort | [OK] EXISTS -- `xiom-graph` crate |
| **A3: Parallel Check** | Type-check independent modules concurrently | [FAIL] NOT DONE |
| **A4: Parallel Codegen** | Emit IR for independent functions concurrently | [FAIL] NOT DONE |
| **A5: Thread-safe Registry** | Make type/function registries concurrent | [OK] DONE -- `SyncRegistry` (Arc<RwLock<HashMap>>) |
...
## 2. DOMAIN B -- LANGUAGE-LEVEL THREADING [OK] (COMPLETE)
...
| Requirement | Status |
|-------------|--------|
| Thread-safe type registry | [OK] SyncRegistry |
| Spawn for parallel compilation | [OK] spawn codegen |
| Channel for pipeline communication | [OK] Channel[T] |
| Thread pool for work distribution | [OK] Thread pool |
| Move semantics (prevent use-after-move in spawn) | [OK] DONE -- capture forwarding via env struct |
| Thread-local state (recursion counter) | [OK] DONE -- `thread_local` |
| Parallel codegen | [OK] DONE -- `--parallel-codegen` with rayon |
| Send/Sync enforcement (prevent data races) | [FAIL] Remaining (Phase B) |
