...
Canonical v0.57 lowering (plan S2.7):
...
(no second trampoline -- the outer SEH checkpoint covers them; S2.13).
```
...
Unsafe Confinement gates (v0.57 -- see S3.1 for full rules):
...
compute-once per constant, minimal lock contention (docs/CTFE_PLAN.md S6, at
...
compile_program():
  ...
  + v0.57: declare @xiom_trampoline_* / @xiom_guard_* (see S9 declares)
...
Expr::Unsafe (v0.57 -- canonical trap lowering, see S3.2):
  ...
  + nested unsafe (in_unsafe_block_fn) -> plain block, no second trampoline (S2.13)
