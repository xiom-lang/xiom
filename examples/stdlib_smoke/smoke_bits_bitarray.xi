...
fn main() -> Int {
  ...
  if b0 != 129 { io.println(string.str_concat("ba to_bytes ", to_string(b0))); return 16; }

  if bitfield.bitfield_get(0xF0, 4, 4) != 0x0F { io.println(string.str_concat("bf get ", to_string(bitfield.bitfield_get(0xF0, 4, 4)))); return 17; }
  if bitfield.bitfield_get(0x12345678, 0, 32) != 0x12345678 { io.println("bf get32"); return 18; }
  if bitfield.bitfield_set(0x0000, 4, 4, 0x0F) != 0x00F0 { io.println(string.str_concat("bf set ", to_string(bitfield.bitfield_set(0x0000, 4, 4, 0x0F)))); return 19; }
  if bitfield.bitfield_clear(0x00F0, 4, 4) != 0 { io.println("bf clear"); return 20; }
  if bitfield.bitfield_sign_extend(0x80, 8) != -128 { io.println(string.str_concat("bf signext ", to_string(bitfield.bitfield_sign_extend(0x80, 8)))); return 21; }
  ...
  if bitwise.popcnt(0xF0) != 4 { io.println(string.str_concat("popcnt ", to_string(bitwise.popcnt(0xF0)))); return 26; }
  if bitwise.popcnt(-1) != 64 { io.println("popcnt -1"); return 27; }
  if bitwise.clz(1) != 63 { io.println(string.str_concat("clz ", to_string(bitwise.clz(1)))); return 28; }
  ...
  if bitwise.bit_reverse_byte(0x01) != 0x80 { io.println(string.str_concat("bit_reverse_byte ", to_string(bitwise.bit_reverse_byte(0x01)))); return 32; }
  ...
  if bitwise.bit_width(255) != 8 { io.println(string.str_concat("bit_width ", to_string(bitwise.bit_width(255)))); return 37; }
  if bitwise.bit_length(0) != 0 { io.println("bit_length 0"); return 38; }
  if bitwise.leading_ones(0xF000000000000000) != 4 { io.println(string.str_concat("leading_ones ", to_string(bitwise.leading_ones(0xF000000000000000)))); return 39; }
  ...
  if rc.0 != 1 { io.println(string.str_concat("rcl ", to_string(rc.0))); return 63; }
